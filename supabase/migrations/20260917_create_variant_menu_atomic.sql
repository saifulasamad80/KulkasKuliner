-- Membuat satu menu dan semua variannya dalam satu transaksi database.
-- RPC ini cuma boleh dipanggil service_role lewat endpoint admin terautentikasi.
...
begin
  ...
  from jsonb_to_recordset(p_variants) as variant(variant_name text, price numeric, stock integer);
  ...
  insert into public.products (name, price, stock, image_url, description, menu_id, variant_name, is_active)
  select
    v_menu_name,
    variant.price,
    variant.stock,
